...
public class ApplyController {
    ...
    public Result user(@PathVariable String user) {
        return Result.success(applyService.getOne(new LambdaQueryWrapper<Apply>().eq(Apply::getUser,user).eq(Apply::getState,"审核通过")));
    }
    ...
    public Result save(@RequestBody Apply apply) {
        Projects projects = projectsService.getById(apply.getProjectsId());

        if (apply.getId() == null) {
            Apply one = applyService.getOne(new LambdaQueryWrapper<Apply>().eq(Apply::getUser, apply.getUser()).in(Apply::getState,"待审核","审核通过"));
            if (one != null){
                return Result.error("201","请勿重复选择");
            }

            projects.setState("已选");
            projectsService.updateById(projects);
        }
        applyService.saveOrUpdate(apply);

        // 如果审核不通过
        if (apply.getState().equals("审核不通过")){
            projects.setState("未选");
            projectsService.updateById(projects);
        }
    ...
    */
    ...
    public void export(HttpServletResponse response) throws Exception {
        ...
        String fileName = URLEncoder.encode("Apply信息表", "UTF-8");
