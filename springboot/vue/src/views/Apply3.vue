<template>
  <div>
    ...
    <div style="margin: 10px 0">
      ...
      <!-- <el-upload action="http://localhost:9090/apply3/import" :show-file-list="false" accept="xlsx" :on-success="handleExcelImportSuccess" style="display: inline-block">
    ...
    <el-dialog title="信息" :visible.sync="dialogFormVisible" width="40%" :close-on-click-modal="false">
      <el-form label-width="140px" size="small" style="width: 85%;" :model="form" :rules="rules" ref="ruleForm">
        ...
        <el-form-item prop="content" label="备注" v-if="user.role === 'ROLE_USER'">
...
export default {
  ...
  methods: {
    ...
    save() {
      ...
      this.request.post("/apply3", this.form).then(res => {
        if (res.code === '200') {
    ...
    del(id) {
      this.request.delete("/apply3/" + id).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.request.post("/apply3/del/batch", ids).then(res => {
        if (res.code === '200') {
    ...
    exp() {
      window.open("http://localhost:9090/apply3/export")
    },
