...
<!--      <el-input style="width: 200px" placeholder="请输入" suffix-icon="el-icon-position" class="ml-5" v-model="address"></el-input>-->
    ...
    <div style="margin: 10px 0">
      ...
      <!-- <el-upload action="http://localhost:9090/apply2/import" :show-file-list="false" accept="xlsx" :on-success="handleExcelImportSuccess" style="display: inline-block">
...
export default {
  ...
  methods: {
    ...
    save() {
      ...
      this.request.post("/apply2", this.form).then(res => {
        if (res.code === '200') {
    ...
    del(id) {
      this.request.delete("/apply2/" + id).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.request.post("/apply2/del/batch", ids).then(res => {
        if (res.code === '200') {
    ...
    exp() {
      window.open("http://localhost:9090/apply2/export")
    },
